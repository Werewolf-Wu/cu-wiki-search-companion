...
GitHub Actions 在每次 push 后执行测试、类型检查和生产构建，并上传带提交号的短期 artifact。main 验证成功后还会更新唯一的滚动 nightly prerelease：同名 tag 与 assets 始终指向最新通过验证的提交，不会为每次 push 新建一条 Release。Release 说明同时列出上一个成功 Nightly 到本次推送之间的短 SHA、提交标题与 compare 链接。
...
- 20 个测试文件、148 项自动化测试通过。
- TypeScript 严格检查和 Vite 生产构建通过。
- 三类快照连续恢复的查询结果与本地全量重建一致，完整正文缓存不会重复请求 revisions。
- Edge 152 连续三次按模式恢复的总耗时为 4.591、4.905、4.505 秒（中位 4.591 秒）；清除快照后的本地全量重建为 8.858 秒，恢复快约 48%。标题阶段正文/Lua 均保持未加载，正文阶段也不会预载 Lua；稳定缓存的首次构建、三次恢复与本地重建均没有发出 revisions 请求。
